...
namespace Expenses.Wcf
{
    public class ExpenseService : IExpenseService
    {
        ...
        public int UpdateExpenseReport(ExpenseReport expenseReport)
        {
            ...
            if (dbExpenseReport.Status == DbExpenseReportStatus.Approved)
            {
                using (var client = new InternalService.InternalServiceClient())
                {
                    client.ProcessReport(dbExpenseReport.ExpenseReportId);
                }
            }
